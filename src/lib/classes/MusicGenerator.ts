...
export default class MusicGenerator {
    ...
    constructor(oscClient){
        ...
        for(let i=0; i<this.noteMarkovTable.length; i++){
            ...
            for(let j=0; j<7; j++){ this.noteMarkovTable[i][j] = 1/7; }
        }
    ...
    forwardSequence(){
        ...
        if(this.currentSequence.length<1){ this.generateNewSequence() }
        if(this.alterSeq && Math.random()<this.alterChance){ this.currentSequence[this.seqCurrentIndex] = this.generateNote(); }
    ...
    generateNote(){
        ...
        if(true){ 
            ...
            let sum = 0;
            for(let i=0; i<markovRow.length; i++){ markovRow[i] = Math.pow(markovRow[i], pow); sum+= markovRow[i]; }
            for(let i=0; i<markovRow.length; i++){ markovRow[i] /= sum; }
        }
        // build cumulative distribution
        let cumulativeRow = new Array(7);
        cumulativeRow[0] = markovRow[0];
        for(let i=1; i<7; i++){ cumulativeRow[i] = markovRow[i]+cumulativeRow[i-1]; }
        ...
        if(this.faceDistance>1 && this.faceDistance<1.7){ if(Math.random()*5<this.faceDistance){note+=12; }}
        else{ if(this.faceDistance>1.7 && Math.random()*10<this.faceDistance){note+=24; }}
        ...
        if(true){ // exaggerate probability
            let pow = 1.5;
            let sum = 0;
            for(let i=0; i<durationMarkovRow.length; i++){ durationMarkovRow[i] = Math.pow(durationMarkovRow[i], pow); sum+= durationMarkovRow[i]; }
            for(let i=0; i<durationMarkovRow.length; i++){ durationMarkovRow[i] /= sum; }
        }
        durationCumulativeProb[0] = durationMarkovRow[0];
        for(let i=1; i<this.noteDuration.length; i++){ durationCumulativeProb[i] = durationMarkovRow[i]+durationCumulativeProb[i-1]; }
        generatedIndex = -1;
        rand = Math.random();
        for(let i=0; i<durationCumulativeProb.length; i++){
            if(rand<durationCumulativeProb[i]){ generatedIndex = i; break; }
        }
